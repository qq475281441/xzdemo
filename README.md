...
# 补充假设信息
1. 系统随机为用户分配相邻的五个座位
1. 假设b区、c区的1-9行座位视野更好实行和其他座位不一样的定价

1. b区1-9行 c区1-9行          299一个座位                       优先级5
1. d区1-9行 a区1-9行          199一个座位                       优先级4
1. b区10-18行 c区10-18行      159一个座位                       优先级3
1. d区10-18行 a区10-18行      99一个座位                         优先级2
1. a/b/c/d区19-26行          49一个座位                         优先级1
...
## 抢购问题：
1. 使用redis队列进行购买操作，解决抢购时的高并发和超售的问题

1. 后台监听订单时间，超过五分钟未支付订单会自动取消

1. 使用wss取代http接口，可以承受更高的并发
